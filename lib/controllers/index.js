...
IndexController.info = function*(){
	yield this.render('index/info');
}

IndexController.diary = function*(){
	var items = [];
	for(var i = 0;i<10;i++){
		items.push({});
	}
	yield this.render('index/diary',{items:items});
}
...
//照片墙
IndexController.photowall = function*(){
	yield this.render('index/photowall');
}	
